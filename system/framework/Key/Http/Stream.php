...
/**
 * Describes a data stream. Typically, an instance will wrap a PHP stream; this interface provides
...
class Stream implements StreamInterface
{
     ...
     * Create a new stream.
     *
     * @param resource $stream A PHP stream handle.
    ...
    public function seek($offset, $whence = SEEK_SET)
    {
        ...
        if (!$this->isReadable() || fseek($this->stream, $offset, $whence) === -1) {
            throw new RuntimeException('Count not seek in the stream');
        }
    }
    ...
    public function write($string)
    {
        if (!$this->isWritable() || ($written = fwrite($this->stream, $string)) === false) {
            throw new RuntimeException('Count not write to the stream');
        }
